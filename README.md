...
└── tools/                                # 开发工具和脚本
    ├── rss_digital_twin.py                # 数字孪生仿真器（完全复刻EnforceScript体力系统逻辑）
    ├── rss_optimizer.py                   # NSGA-II多目标优化器
    ├── rss_optimizer_optuna.py            # Optuna贝叶斯优化器（推荐）
    ├── rss_scenarios.py                   # 标准测试工况库（13个标准工况）
    ├── rss_visualization.py               # 可视化工具集（5种专业图表）
    ├── rss_sensitivity_analysis.py        # 参数敏感性分析工具
    ├── rss_sensitivity_analysis_enhanced.py # 增强版敏感性分析（联合敏感性）
    ├── rss_report_generator.py            # 报告生成器（HTML/Markdown）
    ├── rss_report_generator_english.py    # 英文版报告生成器
    ├── run_complete_optimization.py       # 完整优化流程脚本
    ├── simulate_stamina_system.py         # 基础趋势图生成脚本
    ├── generate_comprehensive_trends.py  # 综合趋势图生成脚本
    ├── multi_dimensional_analysis.py     # 多维度趋势分析脚本
    ├── stamina_constants.py               # Python常量定义
    ├── requirements.txt                  # Python依赖包
    ├── README.md                          # Tools模块完整使用指南
    ├── BUGFIX_SUMMARY.md                  # 问题修复总结
    ├── reports/                           # 生成的报告目录
    │   ├── optimization_report.html       # HTML格式优化报告
    │   └── optimization_report.md         # Markdown格式优化报告
...
RSS Tools 模块是一个完整的参数优化和分析工具集，用于平衡 Arma Reforger 模组的"拟真度"与"可玩性"。

### 核心特性

- ✅ **数字孪生仿真器**：完全复刻 EnforceScript 的体力系统逻辑
- ✅ **标准测试工况库**：13 个标准测试工况（ACFT 2英里测试、Everon拉练、火力突击等）
- ✅ **多目标优化**：支持 NSGA-II 和 Optuna 贝叶斯优化
- ✅ **可视化工具集**：5 种专业可视化图表（帕累托前沿、敏感度热力图、收敛曲线等）
...
完全复刻 EnforceScript 的体力系统逻辑，支持所有核心特性：
...
- 累积疲劳和代谢适应系统
- 游泳体力管理（3D物理模型）
- 环境因子系统（热应激、降雨湿重、风阻、泥泞度、气温）

#### 2. 标准测试工况库（rss_scenarios.py）

13 个标准测试工况：

- ACFT 2英里测试（15:27配速目标）
- Everon 拉练（10度坡度）
- 火力突击（Sprint+休息循环）
- 趴姿恢复测试
- 游泳体力测试
...
#### 4. 可视化工具集（rss_visualization.py）

5 种专业可视化图表：

1. **帕累托前沿散点图**：显示拟真度 vs 可玩性的权衡
2. **参数敏感度热力图**：显示每个参数的变异系数
3. **收敛曲线图**：显示优化过程中的目标函数变化
4. **参数分布图**：显示帕累托前沿上参数的分布
5. **雷达图**：对比不同配置方案的参数
...
问题修复总结请参考：[tools/BUGFIX_SUMMARY.md](tools/BUGFIX_SUMMARY.md)
...
- **v3.6.0** (当前版本) - 姿态转换成本系统 v1.0 与跳跃检测优化
    - **姿态转换成本系统 v1.0（Stance Transition Cost v1.0 - 乳酸堆积模型）**
      ...
      - **负重因子线性化**：使用线性公式（WeightFactor = CurrentWeight / 90.0）
      - **协同保护逻辑**：
...
- **v3.2.0** - 时间单位错误修复
  ...
  - 配置方案推荐：
    ...
    - 休闲服务器：可玩性优先配置
  - 代码统计：
    - 14 个 Python 脚本（约 4700 行）
    - 2 个文档文件（约 920 行）
    - 1 个依赖包配置文件（15 行）

- **v2.16.0** - 环境模拟优化（v2.16.0）
