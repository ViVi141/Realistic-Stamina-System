# Realistic Stamina System (RSS) v6.2.3
...
[![Version](https://img.shields.io/badge/Version-6.2.3-brightgreen)](CHANGELOG.md)
...
**2026-09-01** — See `CHANGELOG.md` **[6.2.3]**  
MP HUD W′ no longer stuck at 100% (client-side Phase B tick; ignore crude server Rpl overwrite); config **6.2.3**.

## v6.2.2 Updates

**2026-09-01** — See `CHANGELOG.md` **[6.2.2]**  
Heuristic null checks; never call GetVelocity/SetVelocity on load; land speed = position delta; config **6.2.2**.

## v6.2.1 Updates

**2026-09-01** — See `CHANGELOG.md` **[6.2.1]**  
First GetVelocity crash attempt (Physics.GetVelocity still AV) → superseded by **6.2.2**.

## v6.2.0 Updates

**2026-08-30** — See `CHANGELOG.md` **[6.2.0]**  
Walk speed-limit denominator / display fixes; W′ empty cruise scales `CharacterForward` (regression: Walk overspeed clamp no longer disables Run cruise analog); PlayerBase tick split; config **6.2.0**.

## v6.1.7 Updates

**2026-08-14** — See `CHANGELOG.md` **[6.1.7]**
Tier presets re-tuned (Elite CP 889.7→907.8 etc.); W′ exhaustion speed cap enabled (CP-cruise via SetSpeedLimit + SetMovementMaxSpeed); wheel (SetDynamicSpeed) overspeed is an accepted engine limitation; config **6.1.7**.

## v6.1.6 Updates

**2026-08-14** — See `CHANGELOG.md` **[6.1.6]**
Math-model audit fixes: per-tier Skiba/linear W′ dispatch; constants convergence (Custom 300× drift fix); env realism (unified thermoneutral band, exponential rain decay, swim double-count fix); EPOC CP unified; loaded-gait tax switch; config **6.1.6**.

## v6.1.5 Updates

**2026-08-13** — See `CHANGELOG.md` **[6.1.5]**
Breath SFX re-enabled with a local Settings toggle; less false panting on easy run; AI stamina calc off by default; config **6.1.5**.

## v6.1.4 Updates

**2026-08-09** — See `CHANGELOG.md` **[6.1.4]**
External API exposes `wPrimeJoules` / `wPrimeMaxJoules`; docs bilingual + developer guide; config **6.1.4**.

## v6.1.3 Updates

**2026-08-09** — See `CHANGELOG.md` **[6.1.3]**
Removed in-mod re-activate for third-party `Enabled 0` stamina prefabs (compat moves to a separate mod); config **6.1.3**.

## v6.1.2 Updates

**2026-08-09** — See `CHANGELOG.md` **[6.1.2]**  
Full-pool W′→engine STA (native sway) + Exhaustion from presentation; breath SFX off by default; config **6.1.2**.

## v6.1.1 Updates

**2026-07-26** — See `CHANGELOG.md` **[6.1.1]**  
Fix Walk + mouse-wheel `DynamicSpeed` (removed fake-Walk clear); dead-path cleanup under drain-only; config **6.1.1**.

## v6.1.0 Updates

**2026-07-26** — See `CHANGELOG.md` **[6.1.0]**  
Drain-only defaults (no physics servo); under-CP W′ depth recovery; RSS-CPCR breath drive; W′→engine FX; soft `P−CP` STA tax after disarm; config **6.1.0**.
