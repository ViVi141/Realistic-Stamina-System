...
### 相对 6.2.6 的开销节约（PerfProbe）

基线 **6.2.6 @ `883a051`**（玩家/AI 同全栈）vs **6.3.0 tip**。

**默认配置**（AI 不算消耗）  
- **玩家**：限速全栈 ~48.5→~15，约 **快 3.3× / 省 ~69%**（`UpdateSpeed` 另计 ~6×）。  
- **AI**：同栈 ~48→生产 `03f`≈16.7，约 **快 2.9× / 省 ~65%**。

**AI 打开 drain**（相对 6.2.6 AI 全栈 B≈55）  
- 现估 ~22，约 **快 2.5× / 省 ~60%**。

| 对比 | 6.2.6 | 6.3.0 | 约快 | 约省 |
|------|-------|-------|------|------|
| 默认 · 玩家 A | ~48.5 | ~15 | ~3.3× | ~69% |
| 默认 · AI `03f` | ~48 | 16.7 | ~2.9× | ~65% |
| AI 开 drain vs 旧 B | ~55 | ~22 | ~2.5× | ~60% |

详见 [CHANGELOG.md](CHANGELOG.md) **[6.3.0]**（勿用探针 D/F 当生产数字）。
...
- **相对 6.2.6**：默认下玩家限速栈省 ~69%（~3.3×）、AI 省 ~65%（~2.9×）；AI 开 drain 相对旧全栈省 ~60%（~2.5×）
- **水壶** — 军火库可取，**暂无体力效果**（未删开发产物，供后续体力食物测试）；勿当作补给
- 配置版本 **6.3.0**

## v6.2.5 版本更新 / v6.2.5 Updates

**2026-09-01**（详见 [CHANGELOG.md](CHANGELOG.md) **[6.2.5]**）

- **W′ 空蹲走** — 蹲/趴不再套站立巡航限速；与站走速度拉开
- 配置版本 **6.2.5**

## v6.2.4 版本更新 / v6.2.4 Updates

**2026-09-01**（详见 [CHANGELOG.md](CHANGELOG.md) **[6.2.4]**）

- **HUD 门禁** — 服务器关不再留残缺默认条；双开不再叠两层
- 配置版本 **6.2.4**

## v6.2.3 版本更新 / v6.2.3 Updates

**2026-09-01**（详见 [CHANGELOG.md](CHANGELOG.md) **[6.2.3]**）

- **联机 HUD W′** — 专服客户端本地 tick W′；忽略粗估 Rpl 盖回 100%
- 配置版本 **6.2.3**

## v6.2.2 版本更新 / v6.2.2 Updates

**2026-09-01**（详见 [CHANGELOG.md](CHANGELOG.md) **[6.2.2]**）

- **启发式空引用** — Game/World 交叉 + 二次 GetPhysics；禁止 GetVelocity/SetVelocity
- **测速** — 陆地仅位置差分；SampleEntityVelocity 只返回 fallback
- 配置版本 **6.2.2**
...
- **专服加载崩溃（未彻底）** — 改 Physics.GetVelocity 后仍于 SampleEntityVelocity AV → 见 6.2.2
- 配置版本 **6.2.1**

## v6.2.0 版本更新 / v6.2.0 Updates

**2026-08-30**（详见 [CHANGELOG.md](CHANGELOG.md) **[6.2.0]**）

- **Walk 限速与显示** — 跨相位限速分母修正，消除 Walk 尖峰；日志倍率按 Run 顶归一；Walk 超速缩轴收紧
- **W′ 巡航缩轴** — 见底才锁巡航；缩 `CharacterForward`；修复误伤 Run 巡航导致 `模拟量=off`
- **PlayerBase 拆分** — Tick DTO / CP cruise / overspeed / W′ tick / Phase C finalize 迁出
- 配置版本 **6.2.0**
...
**2026-08-13**（详见 [CHANGELOG.md](CHANGELOG.md) **[6.1.5]**）

- **呼吸音** — 重新开启；Settings 本机 Breath Sounds 开关；收紧假喘门限
- **AI 体力默认关** — `m_bDisableAIStaminaCalc` 默认 true；配置版本 **6.1.5**

## v6.1.4 版本更新 / v6.1.4 Updates

**2026-08-09**（详见 [CHANGELOG.md](CHANGELOG.md) **[6.1.4]**）

- **外部 API** — `wPrimeJoules` / `wPrimeMaxJoules`；文档双语与开发者指南；配置版本 **6.1.4**

## v6.1.3 版本更新 / v6.1.3 Updates

**2026-08-09**（详见 [CHANGELOG.md](CHANGELOG.md) **[6.1.3]**）

- **移除** 第三方体力 `Enabled 0` 兼容层（拟独立模组）；配置版本 **6.1.3**

## v6.1.2 版本更新 / v6.1.2 Updates

**2026-08-09**（详见 [CHANGELOG.md](CHANGELOG.md) **[6.1.2]**）

- **W′→引擎 STA**：全池映射驱动原生枪口晃动；Exhaustion 与表现条同源（屏幕模糊）
- **呼吸音默认关**；配置版本 **6.1.2**
...
- **v6.2.3** - 联机 HUD W′ 恒 100% 修复（见上文 v6.2.3 与 CHANGELOG）
- **v6.2.2** - 启发式空引用，禁止 GetVelocity（见上文 v6.2.2 与 CHANGELOG）
- **v6.2.1** - PhaseA GetVelocity 修复尝试（仍崩于 Physics 测速，见 6.2.2）
...
- **v6.1.1** - Walk/滚轮 DynamicSpeed 修复 + 死路径清理（见上文 v6.1.1 与 CHANGELOG）
