...
GAME_MAX_SPEED = 5.5  # m/s，0kg 冲刺最大速度
...
TARGET_RUN_SPEED_MULTIPLIER = TARGET_RUN_SPEED / GAME_MAX_SPEED
...
MIN_LIMP_SPEED_MULTIPLIER = 1.0 / GAME_MAX_SPEED
...
# ==================== 速度阈值参数 ====================
SPRINT_VELOCITY_THRESHOLD = 5.5  # m/s，Sprint速度阈值
RUN_VELOCITY_THRESHOLD = 3.8  # m/s，Run速度阈值
...
# 公式形状：上坡和陡下坡均会快速衰减；归一化以平地 (S=0) 为 1.0，使 0 kg 平地下达 5.5 m/s
...
# 拟真平衡点：与 C 端保持一致（现实为分钟级，这里是游戏拟真折中）
EPOC_DELAY_SECONDS = 2.0  # EPOC延迟时间（秒）
...
ENV_TEMPERATURE_COLD_THRESHOLD = 5.0  # [HARD] 冷应激阈值（北约STANAG，与 C 一致）
