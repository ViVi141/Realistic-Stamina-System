...
class SCR_RSS_AIStaminaState
{
        ...
        inout ERSS_AIStaminaState prevState)
    {
        ...
        // — 强制恢复：体力 < 5% 且持续 5s —
        if (s < SCR_RSS_Constants.RSS_AI_STATE_COLLAPSE_THRESHOLD &&
            isStationary &&
            timeStationarySec >= SCR_RSS_Constants.RSS_AI_STATE_FORCE_RECOVER_DELAY_SEC)
        {
        ...
        case ERSS_AIStaminaState.FRESH:
            if (s < SCR_RSS_Constants.RSS_AI_STATE_FRESH_DOWN)
                state = ERSS_AIStaminaState.WINDED;
            break;

        case ERSS_AIStaminaState.WINDED:
            if (s < SCR_RSS_Constants.RSS_AI_STATE_WINDED_DOWN)
                state = ERSS_AIStaminaState.FATIGUED;
            else if (s >= SCR_RSS_Constants.RSS_AI_STATE_WINDED_UP)
                state = ERSS_AIStaminaState.FRESH;
            break;

        case ERSS_AIStaminaState.FATIGUED:
            if (s < SCR_RSS_Constants.RSS_AI_STATE_FATIGUED_DOWN)
                state = ERSS_AIStaminaState.EXHAUSTED;
            else if (s >= SCR_RSS_Constants.RSS_AI_STATE_FATIGUED_UP)
                state = ERSS_AIStaminaState.WINDED;
            break;

        case ERSS_AIStaminaState.EXHAUSTED:
            if (s < SCR_RSS_Constants.RSS_AI_STATE_EXHAUSTED_DOWN)
                state = ERSS_AIStaminaState.COLLAPSED;
            else if (s >= SCR_RSS_Constants.RSS_AI_STATE_EXHAUSTED_UP)
                state = ERSS_AIStaminaState.FATIGUED;
            break;
        ...
        case ERSS_AIStaminaState.RECOVERING:
            if (s >= SCR_RSS_Constants.RSS_AI_STATE_RECOVER_TO_EXHAUSTED)
            {
                if (s >= SCR_RSS_Constants.RSS_AI_STATE_RECOVER_TO_FATIGUED)
                    state = ERSS_AIStaminaState.FATIGUED;
                else
                    state = ERSS_AIStaminaState.EXHAUSTED;
            }
