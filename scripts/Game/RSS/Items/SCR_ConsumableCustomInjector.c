// ============================================================
// SCR_ConsumableCustomInjector.c
// 职责: CustomInjector 消耗品 — 持续治疗效果 + 消耗品逻辑 + 用户动作
// 所属域: Items
// ============================================================

// ---------------------------------------------------------------------------
// 1. CustomInjector 持续伤害效果
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 2. CustomInjector 消耗品效果
// ---------------------------------------------------------------------------
[BaseContainerProps()]
class SCR_ConsumableCustomInjector : SCR_ConsumableEffectHealthItems
{
    protected bool PassesCustomInjectorRules(
        notnull ChimeraCharacter character, notnull IEntity user,
        ...
        out SCR_EConsumableFailReason failReason)
    {
        ...
        array<ref SCR_PersistentDamageEffect> effects =
            damageMgr.GetAllPersistentEffectsOfType(SCR_CustomInjectorDamageEffect);
        if (!effects.IsEmpty())
        {
