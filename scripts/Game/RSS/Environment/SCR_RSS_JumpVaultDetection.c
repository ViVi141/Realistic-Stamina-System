...
class SCR_RSS_JumpVaultDetector
{
    // ==================== 状态变量 ====================
    // 跳跃相关
        ...
        if (!isClimbing && hasJumpInput)
        {
            ...
            }
            {
                ...
                if (encumbranceCacheValid)
                    currentTotalWeight = SCR_RSS_MetabolismMath.CHARACTER_WEIGHT + cachedCurrentWeight;
                else
                {
                ...
                float hguess = SCR_RSS_ConfigBridge.GetJumpHeightGuess(); // [SOFT] 重心抬升高度估算
                ...
                if (signalsManager && exhaustionSignal != -1)
                {
                    ...
                    if (currentTotalWeight > SCR_RSS_MetabolismMath.CHARACTER_WEIGHT)
                    {
                        float weightRatio = (currentTotalWeight - SCR_RSS_MetabolismMath.CHARACTER_WEIGHT) / SCR_RSS_MetabolismMath.MAX_ENCUMBRANCE_WEIGHT;
                      ...
                      bool encumbranceCacheValid, float cachedCurrentWeight)
    {
        ...
        if (isClimbing)
        {
            ...
            if (!m_bIsVaulting && m_fLastVaultTime >= 0.0 && (currentTime - m_fLastVaultTime) < SCR_RSS_Constants.VAULT_COOLDOWN_SEC)
            {
            ...
            if (!m_bIsVaulting)
            {
                ...
                float vert = SCR_RSS_Constants.VAULT_VERT_LIFT_GUESS;          // [HARD] 垂直抬升估算 0.5m
            ...
            else
            {
                ...
                if (currentTime - m_fLastContinuousClimbTime >= SCR_RSS_Constants.VAULT_CONTINUOUS_CLIMB_INTERVAL_SEC)
                {
                    float currentTotalWeight = SCR_RSS_MetabolismMath.CHARACTER_WEIGHT;
                    if (encumbranceCacheValid)
                        currentTotalWeight = SCR_RSS_MetabolismMath.CHARACTER_WEIGHT + cachedCurrentWeight;
                    ...
                    float eta_iso = SCR_RSS_ConfigBridge.GetClimbIsoEfficiency();
                    float vertSpeed = SCR_RSS_Constants.VAULT_VERT_LIFT_GUESS;
                    float limbForce = currentTotalWeight * SCR_RSS_Constants.VAULT_LIMB_FORCE_RATIO;
                    continuousClimbCost = SCR_RSS_MetabolismMath.ComputeClimbCostPhys(
