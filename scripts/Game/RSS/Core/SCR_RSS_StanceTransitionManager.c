...
class SCR_RSS_StanceTransitionManager
{
        ...
        IEntity owner)
    {
        ...
        if (encumbranceCacheValid)
            currentTotalWeight = SCR_RSS_Constants.CHARACTER_WEIGHT + cachedCurrentWeight;
        else
        {
        ...
        if (SCR_RSS_ConfigBridge.IsDebugEnabled() && owner == SCR_PlayerController.GetLocalControlledEntity())
        {
            ...
            PrintFormat("[RSS] 姿态转换！%1 | 消耗: %2%% (基础: %3%%, 负重: %4kg, 倍数: %5, 疲劳堆积: %6)",
                ...
                Math.Round(currentTotalWeight - SCR_RSS_Constants.CHARACTER_WEIGHT).ToString(),
    ...
    protected float GetBaseTransitionCost(ECharacterStance oldStance, ECharacterStance newStance)
    {
        ...
        if (oldStance == ECharacterStance.PRONE && newStance == ECharacterStance.STAND)
            return SCR_RSS_Constants.STANCE_COST_PRONE_TO_STAND; // 1.5% - 趴到站
