...
//! 单 tick 体力消耗率汇总结果（UpdateLoop 调用）
...
class SCR_RSS_UpdateCoordinator
{
    ...
    static StaminaDrainTickResult CalculateTotalDrainRate(StaminaDrainTickParams tick)
    {
        ...
        float metabolicEfficiencyFactor = SCR_RSS_StaminaConsumptionCalculator.CalculateMetabolicEfficiencyFactor(tick.speedRatio);
        float fitnessEfficiencyFactor = SCR_RSS_StaminaConsumptionCalculator.CalculateFitnessEfficiencyFactor();
        ...
        else
        {
            result.totalDrainRate = SCR_RSS_StaminaConsumptionCalculator.CalculateStaminaConsumption(
                ...
                tick.isSprinting,
                tick.currentMovementPhase);
            result.totalDrainRate = result.totalDrainRate * tick.heatStressMultiplier;
        }
        ...
        float timeDeltaSeconds = 0.2)
    {
        ...
        if (!isInEpocDelay)
        {
            ...
            recoveryRate = SCR_RSS_RecoveryCalculator.CalculateRecoveryRate(
                staminaPercent,
        ...
        float currentSpeed)
    {
        ...
        if (inVehicle)
        {
            ...
            s_pRecoveryCtx.staticDrainForRecovery = -SCR_RSS_Constants.REST_RECOVERY_PER_TICK;
        ...
        else
        {
            ...
            s_pRecoveryCtx.currentWeightForRecovery = SCR_RSS_RecoveryCalculator.CalculateRecoveryWeight(currentWeightForRecovery, controller);
