...
class SCR_RSS_SwimmingStaminaModel
{
    ...
    static float CalculateSwimmingStaminaDrain(float velocity, float currentWeight)
    {
        ...
        // ==================== 1. 静态消耗优化（踩水维持）====================
        // 将基础功率降低，模拟更高效的生存踩水
        float baseMaintenancePower = SCR_RSS_SwimConstants.SWIMMING_BASE_POWER; // 25W（从50W降至25W）
        
        float staticMultiplier = 1.0;
        // 使用 25kg 作为硬核负重的起点
        float threshold = SCR_RSS_SwimConstants.SWIMMING_ENCUMBRANCE_THRESHOLD; // 25kg
        
        if (effectiveWeight > threshold)
        {
            // 只有超过 threshold 之后才开始指数级增加
            // 模拟：负重越大，为了维持头部在水面上所需的功率呈非线性上升
            float loadFactor = (effectiveWeight - threshold) / (SCR_RSS_SwimConstants.SWIMMING_FULL_PENALTY_WEIGHT - threshold);
            loadFactor = Math.Clamp(loadFactor, 0.0, 1.0);
            
            // 使用平方律：让 25-30kg 之间依然相对温和，35kg之后才真正痛苦
            const float powerExponent = 2.0; // 平方律
            staticMultiplier = 1.0 + (Math.Pow(loadFactor, powerExponent) * (SCR_RSS_SwimConstants.SWIMMING_STATIC_DRAIN_MULTIPLIER - 1.0));
        }
        ...
        if (velocity > SCR_RSS_SwimConstants.SWIMMING_MIN_SPEED)
        {
            float vClamped = Math.Clamp(velocity, 0.0, 7.0);
            float velocityCubed = vClamped * vClamped * vClamped; // v³
            dynamicPower = 0.5 * SCR_RSS_SwimConstants.SWIMMING_WATER_DENSITY * velocityCubed * 
            ...
            // 但为了游戏性平衡，我们取一个折中值
            dynamicPower = dynamicPower * SCR_RSS_SwimConstants.SWIMMING_DYNAMIC_POWER_EFFICIENCY;
        ...
        float totalPower = staticPower + dynamicPower + SCR_RSS_SwimConstants.SWIMMING_SURVIVAL_STRESS_POWER;
        
        // 生理功率上限：防止异常速度/模组冲突导致 v³ 产生天文数字并在后续乘法中溢出
        totalPower = Math.Clamp(totalPower, 0.0, SCR_RSS_SwimConstants.SWIMMING_MAX_TOTAL_POWER);
        
        // ==================== 转换为体力消耗率 ====================
        // 直接使用小数系数，不要乘以 100（这是关键修复）
        // ENERGY_TO_STAMINA_COEFF 本身就是为了将功率转换为 0.0-1.0 这种小数比例而设计的
        float swimmingDrainRate = totalPower * SCR_RSS_SwimConstants.SWIMMING_ENERGY_TO_STAMINA_COEFF;
        
        // ==================== 4. 低强度折扣（survival floating）====================
        // 关键修正：对于极低强度的踩水（静态功率），引入"有氧代谢效率"
        // 模拟士兵在水中通过深呼吸调整，不完全依赖消耗体力槽
        if (velocity < SCR_RSS_SwimConstants.SWIMMING_LOW_INTENSITY_VELOCITY && effectiveWeight < threshold)
        {
            swimmingDrainRate = swimmingDrainRate * SCR_RSS_SwimConstants.SWIMMING_LOW_INTENSITY_DISCOUNT; // 浮水且负重轻时，额外给予30%的消耗折扣
        }
    ...
    static float CalculateSwimmingStaminaDrain3D(vector velocityVector, float currentWeight)
    {
        ...
        // ==================== A. 静态/生存功率 ====================
        float baseMaintenancePower = SCR_RSS_SwimConstants.SWIMMING_BASE_POWER; // 25W
        float threshold = SCR_RSS_SwimConstants.SWIMMING_ENCUMBRANCE_THRESHOLD; // 25kg
        ...
        if (effectiveWeight > threshold)
        {
            float loadFactor = (effectiveWeight - threshold) / (SCR_RSS_SwimConstants.SWIMMING_FULL_PENALTY_WEIGHT - threshold);
            loadFactor = Math.Clamp(loadFactor, 0.0, 1.0);
            staticMultiplier = 1.0 + (Math.Pow(loadFactor, 2.0) * (SCR_RSS_SwimConstants.SWIMMING_STATIC_DRAIN_MULTIPLIER - 1.0));
        }
        float staticPower = baseMaintenancePower * staticMultiplier;
        ...
        if (vH > SCR_RSS_SwimConstants.SWIMMING_MIN_SPEED)
        {
            ...
            // 人体效率/游戏平衡折中
            horizontalPower = horizontalPower * SCR_RSS_SwimConstants.SWIMMING_DYNAMIC_POWER_EFFICIENCY;
        ...
        if (absVY > SCR_RSS_SwimConstants.SWIMMING_VERTICAL_SPEED_THRESHOLD)
        {
            // 垂直阻力功率：P = 0.5 * rho * |v|^3 * Cd * A
            float vYCubedAbs = absVY * absVY * absVY;
            float verticalDragPower = 0.5 * SCR_RSS_SwimConstants.SWIMMING_WATER_DENSITY * vYCubedAbs *
                                      SCR_RSS_SwimConstants.SWIMMING_VERTICAL_DRAG_COEFFICIENT * SCR_RSS_SwimConstants.SWIMMING_VERTICAL_FRONTAL_AREA;
            ...
            if (vY > 0.0)
            {
                ...
                float baseUpForce = bodyWeight * 9.81 * SCR_RSS_SwimConstants.SWIMMING_VERTICAL_UP_BASE_BODY_FORCE_COEFF;
                float loadUpForce = effectiveWeight * 9.81 * SCR_RSS_SwimConstants.SWIMMING_EFFECTIVE_GRAVITY_COEFF;
                float upPower = (baseUpForce + loadUpForce) * vY;
                verticalPower = (upPower + verticalDragPower) * SCR_RSS_SwimConstants.SWIMMING_VERTICAL_UP_MULTIPLIER;
            }
            else
            {
                // 下潜：对抗浮力 + 垂直阻力
                // 负重会“抵消”一部分浮力需求：负重越大，下潜越容易
                float buoyancyForce = bodyWeight * 9.81 * SCR_RSS_SwimConstants.SWIMMING_BUOYANCY_FORCE_COEFF;
                float loadReliefForce = effectiveWeight * 9.81 * SCR_RSS_SwimConstants.SWIMMING_VERTICAL_DOWN_LOAD_RELIEF_COEFF;
                ...
                verticalPower = (downPower + verticalDragPower) * SCR_RSS_SwimConstants.SWIMMING_VERTICAL_DOWN_MULTIPLIER;
            }
        }
        
        // ==================== D. 总功率 + 保护 ====================
        float totalPower = staticPower + horizontalPower + verticalPower + SCR_RSS_SwimConstants.SWIMMING_SURVIVAL_STRESS_POWER;
        totalPower = Math.Clamp(totalPower, 0.0, SCR_RSS_SwimConstants.SWIMMING_MAX_TOTAL_POWER);
        
        // 转换为体力消耗率（0.0-1.0 / s）
        float drainRate = totalPower * SCR_RSS_SwimConstants.SWIMMING_ENERGY_TO_STAMINA_COEFF;
        
        // 低强度折扣：几乎不动且负重轻时
        if (vH < SCR_RSS_SwimConstants.SWIMMING_LOW_INTENSITY_VELOCITY && absVY < 0.1 && effectiveWeight < threshold)
            drainRate = drainRate * SCR_RSS_SwimConstants.SWIMMING_LOW_INTENSITY_DISCOUNT;
        
        return Math.Clamp(drainRate, 0.0, SCR_RSS_SwimConstants.SWIMMING_MAX_DRAIN_RATE);
    }
}