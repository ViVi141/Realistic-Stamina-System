modded class SCR_CharacterControllerComponent
{
    ...
    {
        RSS_StaminaTickFinalizeContext ctx = new RSS_StaminaTickFinalizeContext();
        ctx.ctrl = this;
        ctx.staminaComponent = m_pStaminaComponent;
        ctx.anaerobicBurst = m_pAnaerobicBurst;
        ctx.staminaState = m_pStaminaState;
        ctx.environmentFactor = m_pEnvironmentFactor;
        ctx.encumbranceCache = m_pEncumbranceCache;
        ctx.fatigueSystem = m_pFatigueSystem;
        ctx.exerciseTracker = m_pExerciseTracker;
        ctx.epocState = m_pEpocState;
        ctx.cardioDrive = m_pCardioDrive;
        ctx.uiSignalBridge = m_pUISignalBridge;
        ctx.aiManager = m_pAIManager;
        ctx.terrainDetector = m_pTerrainDetector;
        ctx.stanceTransitionManager = m_pStanceTransitionManager;
        ctx.appliedSpeedLimitMs = m_fAppliedSpeedLimitMs;
        ctx.landPositionDeltaSpeedMs = m_fLandPositionDeltaSpeedMs;
        ctx.currentWetWeight = m_fCurrentWetWeight;
        ctx.lastCapRatchetPerTick = m_fLastCapRatchetPerTick;
        ctx.lastSpeedSource = m_sLastSpeedSource;
        ctx.sprintGateEnginePokeActive = m_bSprintGateEnginePokeActive;
        ctx.swimmingVelocityDebugPrinted = m_bSwimmingVelocityDebugPrinted;
        ctx.lastStaminaUpdateTime = m_fLastStaminaUpdateTime;
        ctx.lastStaminaPercent = m_fLastStaminaPercent;
        ctx.lastSpeedMultiplier = m_fLastSpeedMultiplier;

        bool ok = SCR_PlayerBaseStaminaTickFinalize.Run(loc, ctx);

        m_bSwimmingVelocityDebugPrinted = ctx.swimmingVelocityDebugPrinted;
        m_fLastStaminaUpdateTime = ctx.lastStaminaUpdateTime;
        m_fLastStaminaPercent = ctx.lastStaminaPercent;
        m_fLastSpeedMultiplier = ctx.lastSpeedMultiplier;
        return ok;
    }
    ...
    //! CP 巡航 / Walk 覆盖 / 限速斜率 / 坡度 EMA（见 SCR_PlayerBaseCpCruiseController）
    protected ref SCR_PlayerBaseCpCruiseController m_pCpCruise;
    ...
    {
        if (!m_pCpCruise)
            m_pCpCruise = new SCR_PlayerBaseCpCruiseController();
        return m_pCpCruise;
    }
    ...
    {
        GetCpCruise().SetRunCruiseCapMs(capMs);
    }

    float RSS_GetRunCruiseCapMs()
    {
        return GetCpCruise().GetRunCruiseCapMs();
    }

    bool RSS_IsCpWalkOverrideActive()
    {
        return GetCpCruise().IsCpWalkOverrideActive();
    }

    float RSS_GetLastMoveAnalog()
    {
        return GetCpCruise().GetLastMoveAnalog();
    }

    void RSS_UpdateObservedFullRunMs(ActionManager am)
    {
        GetCpCruise().UpdateObservedFullRunMs(
            this, am, m_fStatusLogSpeed, m_pAnaerobicBurst, m_pCompartmentAccess);
    }

    void RSS_TryScaleMoveActionValues(ActionManager am, float dt, bool updateServo)
    {
        GetCpCruise().TryScaleMoveActionValues(
            this, am, dt, updateServo, m_fAppliedSpeedLimitMs, m_fStatusLogSpeed,
            m_pAnaerobicBurst, m_pCompartmentAccess);
    }

    void RSS_TryApplyRunCruiseMovementAnalog()
    {
        GetCpCruise().TryApplyRunCruiseMovementAnalog(
            this, m_fAppliedSpeedLimitMs, m_pAnaerobicBurst, m_pCompartmentAccess);
    }

    void RSS_UpdateCpOutOfBandWalkOverride(bool isSwimming, bool isInVehicle, bool holdingMove, bool wPrimeEmpty)
    {
        GetCpCruise().UpdateCpOutOfBandWalkOverride(
            this, isSwimming, isInVehicle, holdingMove, wPrimeEmpty, GetSpeedUpdateIntervalMs());
    }

    void RSS_ReleaseCpWalkOverride()
    {
        GetCpCruise().ReleaseCpWalkOverride(this);
    }

    float RSS_SlewSpeedLimitFraction(float targetFrac, float currentTimeSec)
    {
        return GetCpCruise().SlewSpeedLimitFraction(
            targetFrac, currentTimeSec, m_fLastRssSpeedMultiplierApplied);
    }

    float RSS_GetSmoothedGradePercentForSpeed()
    {
        return GetCpCruise().GetSmoothedGradePercentForSpeed();
    }

    float RSS_SmoothGradePercentForSpeed(float rawGradePercent, float currentTimeSec)
    {
        return GetCpCruise().SmoothGradePercentForSpeed(rawGradePercent, currentTimeSec);
    }
    ...
    override void OnPrepareControls(IEntity owner, ActionManager am, float dt, bool player)
    {
        ...
        if (isLocalOwner)
        {
            GetCpCruise().ResetLastMoveAnalog();
    ...
    void RSS_PokeEngineStaminaForSprintBlock(bool sprintIntent)
    {
        bool pokeActive = m_bSprintGateEnginePokeActive;
        float aerobic01 = GetRssAerobicPercent();
        if (m_pStaminaComponent)
