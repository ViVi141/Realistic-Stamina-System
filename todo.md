...
- 通过 `CharacterController.IsCrouching()` 和 `IsProne()` 获取姿态
- 给 Pandolf 公式增加一个姿态乘数：`k_crouch = 1.6`, `k_prone = 3.0`
...
- 引入一个 3-5 秒的恢复延迟
- 当你狂奔后停下，心率不会立刻下降
- 停下后的前几秒应该维持高代谢水平（EPOC，过量耗氧），延迟 3 秒后再开始计算正向体力恢复
- 在速度判定为 0 时，启动一个 `CallLater` 计时器，计时器结束后才切换 `isRecovering` 标志

**实现难度**: ⭐⭐ (中等)  
**预期代码量**: 30-50 行  
**文件位置**: `scripts/Game/PlayerBase.c`

**注意**: 可能与现有的恢复启动延迟机制（负重下恢复需静止 3 秒）相关，需要整合。
...
- 🔴 高优先级: 0 个（❌ 已全部取消）
- 🟡 中优先级: 7 个（✅ 2 个已完成 - 趴下休息负重优化、模块化重构，5 个待开始：姿态交互、环境应激、恢复延迟、伤病交互、游泳管理、跳跃检测改进）
- 🟢 低优先级: 3 个（鞋具因子、装备重心、文档更新）

**按难度分类**:
- ⭐ 低难度: 1 个（✅ 1 个已完成 - 趴下休息负重优化）
- ⭐⭐ 中等难度: 6 个（✅ 2 个已完成 - 参数校准、模块化重构，4 个待开始：姿态交互、环境应激、恢复延迟、跳跃检测改进）
- ⭐⭐⭐ 高难度: 4 个（❌ 2 个已取消 - 武器稳定性、肾上腺素补偿，4 个待开始：伤病交互、游泳管理）
