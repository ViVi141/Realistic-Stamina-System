...
对照基线：**6.2.6 @ `883a051`**。优化侧：**6.3.0 tip** 重测（`Run` / `RunNearestAi`，iters=3000；日志 2026-09-05）。约快 = 6.2.6 ÷ 现测。

**读表要点**
- **玩家生产**看 `01i` / A / B。
- **AI 默认生产**看 **`03f_apply_cheap_ai`**（含 Tobler + 应用层），**不要**再用 D≈1.7。
- **AI 开消耗**尚无单独合成项；估 ≈ `03f + (F − D)` ≈ **22 µs**（F 仍用旧 cheapAbs，未含 Tobler）。
- D / F 仅保留作探针对照（简化合成），不等于当前 tick。

| 场景 | 6.2.6 | 6.3.0 现测 | 约快 |
|------|-------|-----------|------|
| `UpdateSpeed`（玩家） | 44 | **7.3** | ~6× |
| `UpdateSpeed`（AI 实体上测，生产不走） | 44 | 7.0 | — |
| `GradePercent`（纯 tan / 无室内射线） | 37 | **~0–0.3** | ~∞ |
| A 限速全栈（terrain+env+UpdateSpeed+Apply） | ~48.5 | **~14.7–15** | ~3.3× |
| B 全量（A+代谢+CP） | ~55 | **21** | ~2.6× |
| **AI 默认限速（生产 `ApplyCheapAiSpeed`）** | =A ~48 | **`03f` ≈ 16.7** | **~2.9×** |
| AI 开消耗（估） | =B ~55 | **~22** | **~2.5×** |
| D 探针简化限速（非生产） | — | 1.7 | （勿当实装） |
| F 探针简化管线（非生产） | — | 7.0 | （勿当实装） |

相对 6.2.6 仍明显更快；旧文「AI ~30× / ~9×」来自把 D/F 当成生产路径，**已作废**。
