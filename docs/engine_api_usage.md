...
>
> **v6.3.0（当前）**：限速经 `SCR_RSS_SpeedBridge` → `SetSpeedLimit`（与灌木等 min 合并）。有氧权威在 `m_fTargetStamina`，经 `SCR_StaminaOverride` 拦截壳写入。W′ **不**改有氧权威，但默认经 `SCR_RSS_SprintGate` → `ApplyTransientEngineStamina` 驱动原生晃动/模糊（`Exhaustion`）。坡度/测速/地形优先见 `SCR_RSS_EngineReuse`。编码规范见 [`RSS_CODING_STANDARDS.md`](RSS_CODING_STANDARDS.md)。
>
