...
> **状态：历史设计稿（归档）** — 2026-05-15 草案。类名、群组机动 / GroupSync / 代理等路径均已过时或已从仓库移除。  
> **现行行为说明**请以 [`RSS_AI_行为说明.md`](RSS_AI_行为说明.md) 与 `scripts/Game/RSS/AI/` 源码为准。本文仅保留设计原则与意图备忘，勿按文中路径直接改代码。

> 作者：系统设计  
> 日期：2026-05-15  
> 原状态标注：部分实现（已由现行 AI 模块替代）
