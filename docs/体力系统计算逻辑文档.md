...
**时间线**:
- **0～8 秒**：爆发期。负重速度惩罚 × `TACTICAL_SPRINT_BURST_ENCUMBRANCE_FACTOR`（0.2），即惩罚降至 20%
- **8～13 秒**：缓冲区。负重惩罚从 0.2 倍线性过渡到 1.0 倍
- **13 秒后**：平稳期。按正常 Sprint 负重惩罚计算；并写入冷却开始时间
- **冷却**：爆发+缓冲结束或松开冲刺后，`TACTICAL_SPRINT_COOLDOWN`（15 秒）内再次冲刺不触发爆发，直接按平稳期

**判定**:
- 进入 Sprint 时：若当前时间 ≥ 上次冷却结束时间，则记录 `m_fSprintStartTime`，本段冲刺可享受爆发/缓冲
- 否则不更新 `m_fSprintStartTime`，本段冲刺无爆发（直接平稳期）
