...
体重百分比 = 有效负重 / 角色体重
速度惩罚 = GetEncumbranceSpeedPenaltyCoeff() × 体重百分比^GetEncumbranceSpeedPenaltyExponent()
速度惩罚 = clamp(速度惩罚, 0.0, GetEncumbranceSpeedPenaltyMax())  # 上限由预设决定（约0.56~0.94）
```
注：系数、指数、上限均由预设动态获取；Sprint 时惩罚额外 ×1.5。

**示例计算**（系数0.20、指数1.23，EliteStandard）:
- 30kg负重 (有效负重 ≈ 28.64kg, 体重百分比 ≈ 0.318)
- 速度惩罚 ≈ 0.20 × 0.318^1.23 ≈ 0.048 (减少约4.8%速度，Sprint 时再 ×1.5)
...
### 3.14 坡度修正（*CalculateGradeMultiplier 当前未调用*）

> **注意**：`CalculateGradeMultiplier` 存在于代码中但未被消耗逻辑调用。当前陆地消耗的坡度影响完全由 **Pandolf 坡度项** + **Santee 下坡修正**（3.15）承担。以下公式仅供该函数实现参考。

**上坡**（gradePercent > 0，如 5 表示 5%）:
```
if gradePercent > 0:
    normalizedGrade = gradePercent × 0.01
...
**下坡（缓坡）**:
```
elif gradePercent < 0 and abs(gradePercent) <= 15:
    kGrade = 1.0 + gradePercent × GRADE_DOWNHILL_COEFF  # 0.05
...
```
    ...
    steepGradePenalty = (abs(gradePercent) - 15.0) × 0.02
...
| `CAM_INERTIA_OVERSHOOT_EXPONENT` | 1.5 | 过冲随负重的幂指数（&gt;1 重装更敏感） | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
