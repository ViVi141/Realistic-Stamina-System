...
- **消耗侧**：坡度由 **Pandolf 公式内 G 项** + **Santee 下坡修正** 统一处理，见 `CalculatePandolfEnergyExpenditure`（SCR_RealisticStaminaSystem.c 约 882–948 行）。
- **速度侧**：坡度对目标速度的影响由 **Tobler 徒步函数** 处理，见 `CalculateSlopeAdjustedTargetSpeed`（约 1000–1060 行）。

历史上存在独立函数 `CalculateGradeMultiplier`，已弃用并**从代码中删除**；坡度仅通过上述两处生效，避免重复惩罚或误用。
...
> **说明**：`CalculateGradeMultiplier` 已从代码中删除。当前陆地消耗的坡度影响完全由 **Pandolf 公式内 G 项** + **Santee 下坡修正**（3.15）承担；速度侧由 **Tobler 徒步函数**（3.16）处理。详见本章开头的「坡度影响数据流」。
