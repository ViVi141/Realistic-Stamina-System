...
└── 镜头惯性与头部物理 (CharacterCamera1stPerson.c，modded CharacterCamera1stPerson)
```
...
这样消耗与速度使用的坡度与“沿路径的坡度”一致，避免法线全幅导致的不合理。**文件位置**：[SCR_SpeedCalculation.c](scripts/Game/Components/Stamina/SCR_SpeedCalculation.c)（`GetSlopeProjectionCos`、`GetRawSlopeAngle`）。
...
**文件位置**: [SCR_EncumbranceCache.c](scripts/Game/Components/Stamina/SCR_EncumbranceCache.c)（UpdateCache 内分段计算）
...
**文件位置**: [SCR_RealisticStaminaSystem.c](scripts/Game/Components/Stamina/SCR_RealisticStaminaSystem.c) - `CalculatePandolfEnergyExpenditure` 内坡度项与惩罚项计算；[SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) - 上述常量定义。
...
**文件位置**: [SCR_RealisticStaminaSystem.c](scripts/Game/Components/Stamina/SCR_RealisticStaminaSystem.c) - `CalculateSlopeAdjustedTargetSpeed`
...
**文件位置**: [SCR_SlopeSpeedTransition.c](scripts/Game/Components/Stamina/SCR_SlopeSpeedTransition.c)
...
**文件位置**: [PlayerBase.c](scripts/Game/PlayerBase.c)（冲刺开始时间与冷却）、[SCR_SpeedCalculation.c](scripts/Game/Components/Stamina/SCR_SpeedCalculation.c)（爆发/缓冲期负重减免）、[SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c)（常量）
...
**文件位置**: [SCR_SpeedCalculation.c](scripts/Game/Components/Stamina/SCR_SpeedCalculation.c)（`GetRawSlopeAngle`）、[SCR_StaminaUpdateCoordinator.c](scripts/Game/Components/Stamina/SCR_StaminaUpdateCoordinator.c)（`UpdateSpeed` 内楼梯判定与惩罚减免）
...
**文件位置**: [CharacterCamera1stPerson.c](scripts/Game/Character/CharacterCamera1stPerson.c)、[SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c)（镜头与头部物理常量）
...
| `SPRINT_ENABLE_THRESHOLD` | 0.15 (15%) | Sprint最低体力要求 | [SCR_StaminaConstants.c#L46](f\scripts\Game\Components\Stamina\SCR_StaminaConstants.c#L46) |
...
| 常量/配置 | 默认值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `POSTURE_STAND_MULTIPLIER` | 1.0x | 站姿消耗倍数（基准） | [SCR_StaminaConstants.c](f\scripts\Game\Components\Stamina\SCR_StaminaConstants.c) |
...
| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `SLOPE_UPHILL_COEFF` | 0.08 | 上坡影响系数 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `SLOPE_DOWNHILL_COEFF` | 0.03 | 下坡影响系数 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `SLOPE_MAX_MULTIPLIER` | 2.0 | 最大坡度影响倍数 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `SLOPE_MIN_MULTIPLIER` | 0.7 | 最小坡度影响倍数 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `GENTLE_DOWNHILL_GRADE_MAX` | 12.0 | 缓下坡上限（\|坡度%\|\），此范围内放大省能 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `GENTLE_DOWNHILL_SAVINGS_MULTIPLIER` | 1.25 | 缓下坡时坡度项省能放大倍数 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `STEEP_DOWNHILL_GRADE_THRESHOLD` | 15.0 | 超过此 \|坡度%\|\ 开始加陡下坡刹车惩罚 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `STEEP_DOWNHILL_PENALTY_MAX_FRACTION` | 0.5 | 陡下坡惩罚上限（相对 baseTerm），-30% 时约达此比例 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `UPHILL_SPEED_BOOST` | 1.15 | 上坡目标速度倍率（Tobler 结果再乘） | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `DOWNHILL_SPEED_BOOST` | 1.15 | 下坡目标速度倍率 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `DOWNHILL_SPEED_MAX_MULTIPLIER` | 1.25 | 下坡速度倍率上限 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `INDOOR_STAIRS_ENCUMBRANCE_SPEED_FACTOR` | 0.4 | 室内楼梯下负重速度惩罚乘数 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |

#### 4.1.12 战术冲刺常量

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `TACTICAL_SPRINT_BURST_DURATION` | 8.0 秒 | 爆发期时长 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `TACTICAL_SPRINT_BURST_BUFFER_DURATION` | 5.0 秒 | 爆发结束后缓冲时长（负重惩罚线性过渡到 1.0） | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `TACTICAL_SPRINT_BURST_ENCUMBRANCE_FACTOR` | 0.2 | 爆发期内负重惩罚乘数（0.2 = 惩罚降至 20%） | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `TACTICAL_SPRINT_COOLDOWN` | 15.0 秒 | 爆发/缓冲结束或松开冲刺后冷却时长，期间再次冲刺不触发爆发 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |

#### 4.1.12a 镜头惯性与头部物理常量

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `CAM_INERTIA_START_LAG_DURATION` | 0.25 秒 | 起步滞后时长 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_INERTIA_START_TILT_DEG` | 2.0 度 | 起步时镜头前倾角度 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_INERTIA_DECEL_OVERSHOOT_DURATION` | 0.4 秒 | 急停前冲持续时间 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_INERTIA_DECEL_OVERSHOOT_FORWARD_M` | 0.06 米 | 急停时镜头前冲距离上限（在最大负载时达到） | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_INERTIA_OVERSHOOT_LOAD_MIN_KG` | 10.0 | 过冲曲线：低于此负重（kg）几乎无过冲 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_INERTIA_OVERSHOOT_LOAD_MAX_KG` | 40.0 | 过冲曲线：达到此负重时过冲为最大值 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_INERTIA_OVERSHOOT_EXPONENT` | 1.2 | 过冲随负重的幂指数（1.0–1.2 减轻轻/重装手感差异） | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_INERTIA_ENCUMBRANCE_THRESHOLD` | 0.25 | 负重占比超过此值才开始明显惯性（起步前倾等） | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_BOB_VERTICAL_AMPLITUDE_BASE` | 0.008 米 | 基础垂直颠簸幅度 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_BOB_VERTICAL_FREQ_BASE` | 1.8 Hz | 基础步频，负重时略降 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_BOB_ENCUMBRANCE_SCALE` | 2.0 | 负重对颠簸幅度的缩放 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_BOB_STAMINA_SCALE` | 0.5 | 低体力时额外加深颠簸 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_BOB_UPHILL_SWAY_AMPLITUDE` | 0.012 米 | 上坡左右摇摆幅度 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_BOB_UPHILL_SWAY_FREQ` | 0.6 Hz | 上坡摇摆频率（低频） | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_BOB_UPHILL_SLOPE_DEG_MIN` | 8.0 度 | 坡度超过此值才启用上坡摇摆 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_DEBUG_STRENGTH` | 1.0 | 镜头惯性/颠簸整体倍率。**验证效果时改为 3.0～5.0** 可明显看出是否生效，确认后改回 1.0 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_SPRINT_FOV_BURST_DEG` | 5.0 度 | 爆发期 (0~8s) FOV 加宽 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_SPRINT_FOV_CRUISE_DEG` | 3.0 度 | 稳定期/巡航期 (8s 后) FOV 轻度加宽 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_SPRINT_FOV_LIMP_DEG` | -2.0 度 | 疲劳/Limp 时 FOV 略收窄 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_SPRINT_FOV_LIMP_STAMINA_THRESHOLD` | 0.2 | 低于此体力视为 Limp（FOV 收窄 + 呼吸落步检测） | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_SPRINT_FOV_BONUS_DEG` | 5.0 度 | [保留] 兼容用，实际以 BURST/CRUISE/LIMP 为准 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_SPRINT_FOV_BLEND_UP_SEC` | 0.2 秒 | 进入冲刺时 FOV 过渡到加宽的时长 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_SPRINT_FOV_BLEND_DOWN_SEC` | 0.25 秒 | 退出冲刺时 FOV 过渡回正常的时长 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
| `CAM_SPRINT_FOV_MAX_RATE_DEG_PER_SEC` | 12.0 | FOV 加成变化速率上限（度/秒），全链路无突变 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |

#### 4.1.13 坡度速度过渡常量（SlopeSpeedTransition）

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `TRANSITION_DURATION` | 5.0 秒 | 坡度变化时平滑过渡时长 | [SCR_SlopeSpeedTransition.c](scripts/Game/Components/Stamina/SCR_SlopeSpeedTransition.c) |
| `CHANGE_THRESHOLD` | 0.02 | 目标变化低于此值不触发过渡 | [SCR_SlopeSpeedTransition.c](scripts/Game/Components/Stamina/SCR_SlopeSpeedTransition.c) |
| `SNAP_UP_THRESHOLD` | 0.08 | 目标比当前平滑值高出至少此量时取消平滑、立即提速（避免上坡中细微缓坡频繁加速） | [SCR_SlopeSpeedTransition.c](scripts/Game/Components/Stamina/SCR_SlopeSpeedTransition.c) |
...
| `SPRINT_MAX_SPEED_MULTIPLIER` | 1.0 | Sprint最高速度倍数 | [SCR_StaminaConstants.c](f\scripts\Game\Components\Stamina\SCR_StaminaConstants.c) |
...
| `ENV_TEMPERATURE_HEAT_THRESHOLD` | 30.0 °C | 热应激阈值 | [SCR_StaminaConstants.c](f\scripts\Game\Components\Stamina\SCR_StaminaConstants.c) |
...
| `STANCE_WINDOW_GAP` | 1.0 秒 | 连续两次切换间隔超过此值则结束当前窗口并结算 | [SCR_StaminaConstants.c](scripts/Game/Components/Stamina/SCR_StaminaConstants.c) |
...
实现于 [SCR_StanceTransitionManager.c](scripts/Game/Components/Stamina/SCR_StanceTransitionManager.c)（`TrySettleWindow`、`ProcessStanceTransition`）、[PlayerBase.c](scripts/Game/PlayerBase.c)（体力更新处调用 `TrySettleWindow`）。
