...
- `ENERGY_TO_STAMINA_COEFF`: 9e-07（默认值；运行时可由优化器 JSON 覆盖）

**文件位置**: [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py)

---

### 3.2 Givoni-Goldman跑步模型 *(已弃用；`RSSConstants` 中已无 `GIVONI_*` 字段)*
...
**参数说明**:
- `GIVONI_CONSTANT`: 0.6（历史示例；当前 `RSSConstants` 无此字段）
- `GIVONI_VELOCITY_EXPONENT`: 2.2（历史示例；当前 `RSSConstants` 无此字段）
- `ref`: REFERENCE_WEIGHT (90.0)
- `ENERGY_TO_STAMINA_COEFF`: 9e-07（默认值；运行时可由优化器 JSON 覆盖）
...
**参数说明**:
- `GAME_MAX_SPEED`: 5.5 m/s
- `SPRINT_VELOCITY_THRESHOLD`: 5.5 m/s
...
if is_in_epoc_delay:
    ...
    # EPOC消耗率
    speed_ratio = clip(speed_before_stop / 5.5, 0, 1)
...
以下常量在 Python 孪生中由 `RSSConstants` 数据类承载；**表格为摘要**，若与源码不一致，以 `tools/rss_digital_twin_fix.py` 中 `class RSSConstants` 的当前定义为准（并与 `scripts/Game/RSS/Core/SCR_StaminaConstants.c` 对齐）。

#### 4.1.1 基础游戏参数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `GAME_MAX_SPEED` | 5.5 | 游戏最大速度 (m/s) | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `CHARACTER_WEIGHT` | 90.0 | 角色体重 (kg) | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `REFERENCE_WEIGHT` | 90.0 | 参考体重 (kg) | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

#### 4.1.2 医学模型参数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `PANDOLF_VELOCITY_COEFF` | 3.2 | Pandolf速度系数 (W/kg) | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `PANDOLF_VELOCITY_OFFSET` | 0.7 | Pandolf速度偏移 (m/s) | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `PANDOLF_BASE_COEFF` | 2.7 | Pandolf基础系数 (W/kg) | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `PANDOLF_GRADE_BASE_COEFF` | 0.23 | Pandolf坡度基础系数 (W/kg) | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `PANDOLF_GRADE_VELOCITY_COEFF` | 1.34 | Pandolf坡度速度系数 (W/kg) | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `PANDOLF_STATIC_COEFF_1` | 1.2 | Pandolf静态基础系数 (W/kg) | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `PANDOLF_STATIC_COEFF_2` | 1.6 | Pandolf静态负重系数 (W/kg) | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

#### 4.1.3 能量转换参数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `ENERGY_TO_STAMINA_COEFF` | 9e-07 | 能量到体力转换系数（默认；可由优化器覆盖） | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

#### 4.1.4 恢复系统参数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `BASE_RECOVERY_RATE` | 0.00035 | 基础恢复率（每0.2秒） | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `RECOVERY_NONLINEAR_COEFF` | 0.5 | 恢复非线性系数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `FAST_RECOVERY_DURATION_MINUTES` | 1.5 | 快速恢复期持续时间（分钟） | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `FAST_RECOVERY_MULTIPLIER` | 2.5 | 快速恢复期倍数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `MEDIUM_RECOVERY_DURATION_MINUTES` | 5.0 | 中等恢复期持续时间（分钟） | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `MEDIUM_RECOVERY_MULTIPLIER` | 1.4 | 中等恢复期倍数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `SLOW_RECOVERY_MULTIPLIER` | 0.6 | 慢速恢复期倍数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

#### 4.1.5 姿态恢复倍数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `STANDING_RECOVERY_MULTIPLIER` | 1.5 | 站姿恢复倍数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `CROUCHING_RECOVERY_MULTIPLIER` | 1.5 | 蹲姿恢复倍数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `PRONE_RECOVERY_MULTIPLIER` | 1.8 | 趴姿恢复倍数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

#### 4.1.6 负重惩罚参数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `BASE_WEIGHT` | 1.36 | 基准负重（kg） | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `ENCUMBRANCE_SPEED_PENALTY_COEFF` | 0.20 | 负重速度惩罚系数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `ENCUMBRANCE_STAMINA_DRAIN_COEFF` | 1.5 | 负重体力消耗系数（与当前 `RSSConstants` 默认一致） | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `LOAD_RECOVERY_PENALTY_COEFF` | 0.0001 | 负重恢复惩罚系数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `LOAD_RECOVERY_PENALTY_EXPONENT` | 2.0 | 负重恢复惩罚指数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

#### 4.1.7 阈值参数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `SPRINT_VELOCITY_THRESHOLD` | 5.5 | Sprint速度阈值 (m/s) | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

> `RUN_VELOCITY_THRESHOLD` / `WALK_VELOCITY_THRESHOLD` 仅在 C 端 `SCR_StaminaConstants.c` 定义；Python `RSSConstants` 以 `TARGET_RUN_SPEED`（3.8 m/s）与 `MovementType` 驱动孪生步态，不重复暴露上述阈值字段。

#### 4.1.8 Sprint参数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `SPRINT_STAMINA_DRAIN_MULTIPLIER` | 3.5 | Sprint体力消耗倍数（**已弃用**：C/Python 主线均走 Pandolf，孪生内不乘此系数） | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `SPRINT_SPEED_BOOST` | 0.30 | Sprint速度加成（30%） | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

#### 4.1.9 疲劳参数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `FATIGUE_ACCUMULATION_COEFF` | 0.015 | 疲劳累积系数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `FATIGUE_MAX_FACTOR` | 2.0 | 最大疲劳因子 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

#### 4.1.10 代谢效率参数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `AEROBIC_THRESHOLD` | 0.6 | 有氧阈值 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `ANAEROBIC_THRESHOLD` | 0.8 | 无氧阈值 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `AEROBIC_EFFICIENCY_FACTOR` | 0.9 | 有氧区效率因子 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `ANAEROBIC_EFFICIENCY_FACTOR` | 1.2 | 无氧区效率因子 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

#### 4.1.11 Fitness参数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `FATIGUE_START_TIME_MINUTES` | 5.0 | 疲劳开始时间（分钟） | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `FITNESS_LEVEL` | 1.0 | 健康状态水平 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `FITNESS_EFFICIENCY_COEFF` | 0.35 | 健康状态效率系数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `FITNESS_RECOVERY_COEFF` | 0.25 | 健康状态恢复系数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

#### 4.1.12 姿态移动速度倍数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `POSTURE_CROUCH_MULTIPLIER` | 0.7 | 蹲姿速度倍数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `POSTURE_PRONE_MULTIPLIER` | 0.3 | 趴姿速度倍数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

#### 4.1.13 动作消耗参数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `JUMP_STAMINA_BASE_COST` | 0.035 | 跳跃基础消耗 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `VAULT_STAMINA_START_COST` | 0.02 | 翻越起始消耗 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `CLIMB_STAMINA_TICK_COST` | 0.01 | 持续攀爬消耗 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `JUMP_CONSECUTIVE_PENALTY` | 0.5 | 连续跳跃惩罚系数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

#### 4.1.14 环境参数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `ENV_HEAT_STRESS_MAX_MULTIPLIER` | 1.5 | 热应激最大倍数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `ENV_TEMPERATURE_HEAT_PENALTY_COEFF` | 0.02 | 热应激惩罚系数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `ENV_TEMPERATURE_COLD_RECOVERY_PENALTY_COEFF` | 0.05 | 冷应激恢复惩罚系数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `ENV_SURFACE_WETNESS_PENALTY_MAX` | 0.15 | 地表湿度惩罚最大值 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

#### 4.1.15 高级恢复参数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `MARGINAL_DECAY_THRESHOLD` | 0.8 | 边际效应衰减阈值 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `MARGINAL_DECAY_COEFF` | 1.1 | 边际效应衰减系数 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `MIN_RECOVERY_STAMINA_THRESHOLD` | 0.2 | 最低恢复体力阈值 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `MIN_RECOVERY_REST_TIME_SECONDS` | 3.0 | 最低体力时需要的休息时间（秒） | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

#### 4.1.16 EPOC参数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `EPOC_DELAY_SECONDS` | 0.5 | EPOC延迟时间（秒） | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `EPOC_DRAIN_RATE` | 0.001 | EPOC期间基础消耗率 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |

#### 4.1.17 场景模拟参数

| 常量名 | 数值 | 说明 | 文件位置 |
|--------|------|------|----------|
| `_cache_size` | 10000 | 缓存大小 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `_scenario_cache` | {} | 场景缓存 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
| `max_history_length` | 5000 | 最大历史长度 | [rss_digital_twin_fix.py](tools/rss_digital_twin_fix.py) |
...
- [rss_digital_twin_fix.py](../tools/rss_digital_twin_fix.py) — 数字孪生仿真器，供 `rss_pipeline_v4.py` 调用
- [rss_pipeline_v4.py](../tools/rss_pipeline_v4.py) — v4 主优化流水线（Optuna NSGA-II，三目标）
