...
| **Standard/Tactical** | 线性 `w_prime_recovery_w_per_s` + 分层 cooldown |
...
- **Sprint 门禁**：`IsSprintAllowedWithCp`（有氧 + W′ + cooldown）
...
停跑后延迟期消耗 ∝ **峰值代谢功率**（`EpocState.UpdateExercisePowerSample`），非固定常数。
